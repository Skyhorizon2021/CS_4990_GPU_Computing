...
__global__ void gpu_matrix_mult_tiled(int *a, int *b, int *c, int m, int n, int k) {
    extern __shared__ int shared_mem[];

    int* tile_a = &shared_mem[0];
    int* tile_b = &shared_mem[BLOCK_SIZE * BLOCK_SIZE];

    int row = blockIdx.y * BLOCK_SIZE + threadIdx.y;
    int col = blockIdx.x * BLOCK_SIZE + threadIdx.x;
    int sum = 0;

    for (int sub = 0; sub < (BLOCK_SIZE + n - 1) / BLOCK_SIZE; ++sub) {
        if (row < m && (sub * BLOCK_SIZE + threadIdx.x) < n) {
            tile_a[threadIdx.y * BLOCK_SIZE + threadIdx.x] = a[row * n + sub * BLOCK_SIZE + threadIdx.x];
        } else {
            tile_a[threadIdx.y * BLOCK_SIZE + threadIdx.x] = 0;
        }

        if ((sub * BLOCK_SIZE + threadIdx.y) < n && col < k) {
            tile_b[threadIdx.y * BLOCK_SIZE + threadIdx.x] = b[(sub * BLOCK_SIZE + threadIdx.y) * k + col];
        } else {
            tile_b[threadIdx.y * BLOCK_SIZE + threadIdx.x] = 0;
        }
...
int main(int argc, char const *argv[])
{
    ...
    // Start to count execution time of GPU without tiling
    ...
    printf("%dx%d . %dx%d on GPU without tiling: %f ms.\n\n", m, n, n, k, gpu_elapsed_time_ms);
    //start the GPU version with tiling
        //Start the GPU version with tiling

    CHECK(cudaEventRecord(start, 0));
    size_t shared_mem_size = 2 * BLOCK_SIZE * BLOCK_SIZE * sizeof(int);
    gpu_matrix_mult_tiled<<<dimGrid, dimBlock, shared_mem_size>>>(d_a, d_b, d_c, m, n, k);
    CHECK(cudaDeviceSynchronize());
    CHECK(cudaEventRecord(stop, 0));
    CHECK(cudaEventSynchronize(stop));
    CHECK(cudaEventElapsedTime(&gpu_elapsed_time_ms,start,stop))
    printf("%dx%d . %dx%d on GPU with tiling: %f ms.\n\n", m, n, n, k, gpu_elapsed_time_ms);
    ...
    if (all_ok)
    {
        printf("Result Validated! TEST PASSED\n");
